...
import * as util from '@csrf/core/util'
import { createCsrfMiddleware, ExpressConfig, ExpressTokenOptions } from '@csrf/express'
...
describe('csrfProtectMiddleware integration tests', () => {
	...
	it('adds token to response header', async () => {
		const resp = await request(testApp).get('/').expect(200)
	...
	it('should reject with non-base64 secret', async () => {
		...
		await request(testApp).post('/').set('Cookie', ['_csrfSecret=-']).set('X-CSRF-Token', util.utoa(token)).expect(403)
	})
	...
	it('should reject no secret', async () => {
		...
		await request(testApp).post('/').set('X-CSRF-Token', util.utoa(token)).expect(403)
	})
