...
import { TokenOptions } from '@csrf/core/config'
...
describe('getTokenString', () => {
	...
	it('uses token `value` function when defined', async () => {
		const requestOuter = new Request('http://example.com/', {
			method: 'POST',
...
describe('_hash', () => {
	...
	it('returns known hash for known input', async () => {
		...
		const hash = await util._hash(secret, salt)
		expect(hash).toEqual(
			new Uint8Array([225, 41, 242, 124, 81, 3, 188, 92, 196, 75, 205, 240, 161, 94, 22, 13, 68, 80, 102, 255]),
		)
	})
