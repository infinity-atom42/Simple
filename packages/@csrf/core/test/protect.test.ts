...
import { Config, CookieOptions } from '@csrf/core/config'
import type { Cookie, CsrfProtectArgs } from '@csrf/core/protect'
import { createCsrfProtect, CsrfError } from '@csrf/core/protect'
...
describe('csrfProtect tests', () => {
	...
	describe('exits early when pathname in `excludePathPrefixes`', async () => {
		...
		const urls = ['http://example.com/test-path1/xxx', 'http://example.com/test-path2/xxx']
